...
class UserView(ABC): #абстрактный базовый класс для представлений
    @abstractmethod
    def show_contact(self, record):  #  метод для отображения контента
        pass
    @abstractmethod
    def show_all_contacts(self,record): #  метод для отображения всех контактов
        pass
    @abstractmethod
    def show_help(self): #  метод для отображения всех команд
        pass
class ConsoleUserView(UserView): # реализация консольного интерфейса для отображения данных 
    def show_contact(self, record):
        print(f"name: {record.name.value}")
        phones = ' ,,'.join(phone.value for phone in record.phones) # объединяем все номера телефонов в строку через запятую и печатаем их
        print(f"phones: {phones}")
        if record.birthday: # проверяем установлен ли день рождения
    ...
    def show_all_contacts(self, records):
        for record in records:
            self.show_contact(record) #вызываем show_contact для каждого контакта чтобы отобразить его данные
            print("-" * 20)
    def show_help(self):
        print("Available commands:")
        print("add - add contact")    
        print("change - change contact")
        print("phone - show contact's phone")
        print("all - show all contacts")
        print("add-birthday - add birthday")
        print("show-birthday - show birthday")
        print("birthdays - show nearest birthdays") 
        print("exit - exit")
...
def main():
    ...
    while True:
        ...
        if command == "close" or command == "exit":
            ...
            view.show_all_contacts(book.data.values()) # показывает все контакты
